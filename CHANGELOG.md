...
* Fixed dependency issue for CentOS 10
* Fixed missing Radicale package on RHEL/CentOS 9
* Updated vulnerable prerequisites
* Fixed unavailable download link for Elastic Search
* Updated .NET runtime version
* Fixed incorrect package name issue
* Fixed build-dependencies for the new version of debhelper
* Fixed [Talk issue](https://github.com/ONLYOFFICE/CommunityServer/issues/550)
* Fixed the issue with displaying the scrollbar when the list is empty in Chrome and Edge (Bug 66132).
* Fixed the issue when authorization with 2FA is not possible after restoring on another server (Bug 59626).
* Fixed the error error "Can't restore table projects_comments" when restoring a backup file from a SaaS portal to a server version (Bug 80531).
* Fixed the issue with prevented SSL redirect for IPv6 localhost requests (Bug 79305).
* Corrected domain substitution when configuring SSL (Bug 76454).
* Fixed issue where the portal allowed opening the contents of a file created in the tmp folder (Bug 76563).
* Fixed issue that allowed the transfer of .xml files with XSS vulnerabilities (Bug 76564).
* Fixed the errors when selecting reports related to project tasks (Bug 76419).
* Ensured the max_file_size value in MySQL is inherited correctly when changing the portal memory quota (Bug 76374).
* Fixed issue where the project workspace froze after reloading the page with an active filter (Bug 76137).
* Added validation for the "First Name" and "Last Name" fields (Bug 74219).
* Ensured the "Group" field is displayed in the profile card after blocking a user (Bug 73718).
* Fixed the files/file/:fileId/share method to prevent transferring non-existent permissions (Bug 73320).
* Resolved error in web.log when deleting a user without an avatar (Bug 72508).
* Fixed issue where events were created for the entire day if the specified time fell at midnight (00:00:00 UTC) (Bug 72476).

### Documents module

* Updated empty file templates for the following languages: sq-AL, ca-ES, cs-CZ, da-DK, hu-HU, id-ID, ro-RO, sq-AL, ur-PK 
* Added support for new file formats: Pages, Numbers, Key, MD, HWP, HWPX, VSDX, VSSX, VSTX, VSDM, VSSM, VSTM, TSV, HML
